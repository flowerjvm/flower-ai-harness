...
final class FutureBackedAiModelCall implements AiModelCall {

    private final String callId;
    private final CompletableFuture<AiModelResponse> future;

    FutureBackedAiModelCall(String callId, CompletableFuture<AiModelResponse> future) {
    ...
    public AiModelResponse result() {
        if (poll() != AiModelCallStatus.READY) {
            throw new IllegalStateException("Spring AI call is not ready: " + callId);
        }
        return future.join();
    }
