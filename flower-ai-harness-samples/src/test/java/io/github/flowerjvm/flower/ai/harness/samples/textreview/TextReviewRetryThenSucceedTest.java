...
class TextReviewRetryThenSucceedTest {

    @RegisterExtension
    final AiHarnessTestExtension harness = new AiHarnessTestExtension();

    @Test
    void malformedJsonIsRefinedAndSecondAttemptSucceeds() {
        ...
        assertThat(harness.gateway().recordedCalls().get(1).request().prompt().messages())
                .anySatisfy(message -> assertThat(message.content()).contains("Validation errors"));
    }
}
