...
- **Phase 12+** — provider expansion.
  ...
  `flower-ai-harness-provider-anthropic` have been pulled forward. The
  compatible module is a raw HTTP adapter for OpenAI-compatible
  `/chat/completions` gateways. The OpenAI and Anthropic modules use the
  official provider Java SDKs.
